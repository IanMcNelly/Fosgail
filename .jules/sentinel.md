...
## 2025-05-24 - [XSS Bypass via Control Characters in URLs]
**Vulnerability:** The logic for sanitizing `javascript:` and other malicious protocols in `MarkdownOutput.tsx` was relying solely on `.trim().toLowerCase()`. This is insufficient, as `trim()` only removes standard whitespace (like spaces, tabs, newlines), allowing control characters (e.g., backspace `\b`/`\x08`, null byte `\0`/`\x00`) to be embedded at the beginning of the URI. This causes the `.startsWith('javascript:')` check to return false, bypassing the security check while still potentially allowing XSS in certain browsers/contexts.
**Learning:** `String.prototype.trim()` is not a comprehensive solution for URL sanitization. Malicious inputs can embed hidden control characters to bypass string prefix checks.
**Prevention:** When validating URIs against malicious protocols (e.g., `javascript:`), do not rely solely on `.trim()`. Strip all whitespace and control characters globally (e.g., using `url.replace(/[\s\x00-\x20]+/g, '')`) before checking the protocol to prevent bypasses using embedded control characters.
