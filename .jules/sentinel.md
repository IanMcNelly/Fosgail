...
## 2024-07-27 - Security Enhancement: XSS Bypass using Embedded Control Characters
**Vulnerability:** Cross-Site Scripting (XSS) payload bypass in URL validation. The previous link and image sanitization only used `.trim()` before checking the protocol for `javascript:`, which could be bypassed by inserting control characters like `\x00` inside or immediately before the protocol string (e.g. `java\x00script:alert(1)`).
**Learning:** Checking for malicious protocols (like `javascript:`, `vbscript:`, `data:`) using `.startsWith()` after a simple `.trim()` is insufficient. Browsers strip out ASCII control characters (`\x00-\x20`) from URLs before evaluating the protocol.
**Prevention:** Always globally strip all whitespace and control characters (e.g., using `url.replace(/[\s\x00-\x20]+/g, '')`) from URLs before checking their protocol, or use robust parsing APIs like the `URL` constructor or battle-tested sanitizers like DOMPurify or react-markdown's defaultUrlTransform.
